...
Int_t i,j,k,l,m,pulso,fini=0,ffin=0,tot=0,Draw=0,Reb=1;  			
char title[100], file[100], ci[100]="",bq[100]="";

Float_t y1[N],sum11,sum21,phi1[N],Emax1; 
Int_t I=2000, L=50, G=1, F=2*L+G;
Float_t Gain=3.0;//10000/(2*L);  // Gain in the spectroscopic amplifier
...
switch (argc)		
	...
	case 6: m=sprintf(file,"%s.csv",argv[1]);
		...
		break;		
	case 7: m=sprintf(file,"%s.csv",argv[1]);
		fini=atoi(argv[2]);ffin=atoi(argv[3]);
		L=atoi(argv[4]);G=atoi(argv[5]);
		Gain=atof(argv[6]);
		break;	
	case 8: m=sprintf(file,"%s.csv",argv[1]);
		fini=atoi(argv[2]);ffin=atoi(argv[3]);
		L=atoi(argv[4]);G=atoi(argv[5]);
		Reb=atoi(argv[6]);
		Gain=atof(argv[7]);
		break;		
 	case 9: m=sprintf(file,"%s.csv",argv[1]);
		fini=atoi(argv[2]);ffin=atoi(argv[3]);
		L=atoi(argv[4]);G=atoi(argv[5]);
		Reb=atoi(argv[6]);
		Gain=atof(argv[7]);		
...
for (l=fini;l<(ffin+1);l++) 	
	...
	if(!*in){
		...
		for (j=0;j<(1600+1);j++) 					//////// start to read data for each pulse (each j)
			...
			*in >> ci;//  cout << ci << endl;    			// throw away the first because it is blockXX
			...
			//cout << "Emax1: " << Emax1 << " j= " << j << endl;			// end of XFIR for PULSE 1 ............................
	...
	m=sprintf(title,"Total pulses %d",(tot+1)*1600);
...
if (Draw!=0)
	...
	m=sprintf(title,"MCA_L%dG%d_Ga%f.mca",L,G,Gain);
