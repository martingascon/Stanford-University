...
Float_t x[N],y[N],dx[N],dy[N],y2[N],dy2[N],y3[N],dy3[N],y4[N],dy4[N],ref=1,I1=0,I2=0,I3=0,I4=0,I5=0,I6=0,I7=0,I8=0,I9=0,aux=0; 	
...
Int_t i,k,m,col=1,col2=4,col3=4, col4=4, col5=4;
...
if (col==2)
	...
	la=(HOff+(zero+50)*HRes)*1000000;lb=(HOff+(zero+1800)*HRes)*1000000;
	...
	f1->SetParameter(0,10);	f1->SetParameter(1,21);	         
	f1->SetParameter(2,5); 	f1->SetParameter(3,10);	
	//f1->SetParLimits(0,114.9,115);
	//f1->SetParLimits(1,22.99,23);

	gr0->Fit("f1","QR+");
	gr0->Fit("f1","R");
	d=f1->GetParameter(0); 	f=f1->GetParError(0);  	
	e=f1->GetParameter(1); 	g=f1->GetParError(1);		if (g<0.001) g=0.001;
	o=f1->GetParameter(2); 	p=f1->GetParError(2);	//***************
...
*/
	...
	*out2 << sepa << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << d/(d+o)*100 << "\t" <<  d/(d+o)/100 <<"\t" << o/(d+o)*100<<"\t" << o/(d+o)/100 << endl;
#endif
...
if ((col==4)||(col==5)||(col==6)||(col==8)||(col==10))
	...
	#if FIT 
		TF1 *f1 = new TF1("f1",fitf4,0,120); 				
		f1->SetParameter(0,5865);	
		f1->SetParameter(1,215);	
...
if (argc==6||argc>8)
{
	...
	if(!*in4) 
		...
		while (!in4->eof())
			{
			if (col3==2) *in4 >> xc[i] >> yc[i];
			if (col3==4) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i];  		//if (col==4) *in >> x[i] >> y[i] >> dx[i] >> dy[i];
			if (col3==5) *in4 >> xc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i];
			if (col3==6) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i];  
			if (col3==8) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i] >> y3c[i] >> dy3c[i];
			if (col3==10) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i] >> y3c[i] >> dy3c[i] >> y4c[i] >> dy4c[i];
			i++;
			}
		}
