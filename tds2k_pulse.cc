...
#define N 2483       // Number of points for each pulse (sampling)
...
Int_t i,j,k,m,ini=1,fin=10,tot=10,roia=0,roib=500,temp=0;  				
...
Float_t max[100],area[100];


for (j=ini;j<(fin+1);j++) 	
	{
	m=sprintf(file2,"%s%03d.csv",file,j); 
	...
	if(!*in) 
		...
		for (i=0;i<32;i++) 
			{
			*in >> ci; // take away the header
			sepa=strtok(ci,","); k=0;
			while (sepa != NULL)
				{
				//cout << sepa << " " << k << "=K    i="<<i << endl;
...
Float_t mean_area=0,mean_max=0;
for (i=1;i<11;i++)
	{
	mean_area=mean_area+area[i];
	mean_max=mean_max+max[i];
  	cout << max[i] << endl;	
   	cout << area[i] << endl; 		
	}
mean_max=mean_max/10;
mean_area=mean_area/10;
cout << "max "<< mean_max << endl; 
cout << "area " << mean_area << endl; 
