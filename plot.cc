...
Float_t x[N],y[N],z[N],dx[N],dy[N],y2[N],dy2[N],y3[N],dy3[N],y4[N],dy4[N],ref=1,I1=0,I2=0,I3=0,I4=0,I5=0,I6=0,I7=0,I8=0,I9=0; 	
...
Int_t i,k,m,col=1,col2=4,col3=4;  
char ci[100],title[100]="title",file[100],file2[100],xaxis[100]="x-axis title",yaxis[100]="y-axis title";
char file3[100],file4[100],file5[100],file6[100],file7[100],file8[100],*sepa,latex1[100],tau1[100],tau2[100],Amp1[100],Amp2[100];
char fitf4[100]="[0]+[1]*exp(-(x)*[2])"; //char fitf4[100]="[0]*exp(-x/[1])+[2]*exp(-x/[3])";
...
if(!*in) 
	...
	while (!in->eof())
		{
		if (col==2) *in >> x[i] >> y[i];
		if (col==3) *in >> x[i] >> y[i] >> z[i];
		if (col==4) *in >> x[i] >> dx[i] >> y[i] >> dy[i];  		//if (col==4) *in >> x[i] >> y[i] >> dx[i] >> dy[i];
...
if (col==2)
	...
	gr0->SetTitle(title);					
	gr0->GetXaxis()->SetTitle("Pressure (GPa)");				
	gr0->GetYaxis()->SetTitle("Relative Light Output");				
	...
	tx->SetTextSize(0.05);
	tx->SetTextColor(2);
	tx->DrawLatex(0.8,0.50,"Exp. Data");
	tx->SetTextColor(4);
	tx->DrawLatex(0.8,0.40,"Theoretical");
	}


if (col==3)
	{
	TGraph *gr0b = new TGraph(i-1,x,z);				// Graph declaration
	gr0b->SetLineColor(2);					
...
if ((col==4)||(col==5)||(col==6)||(col==8)||(col==10))
	...
	if ((col==5)||(col==6)||(col==8)||(col==10)) 	
		...
		if (col>5)	 	
			{
			TGraphErrors *gr2 = new TGraphErrors(N,x,y2,dx,dy2);		// Graph declaration
			gr2->SetLineColor(4);						// Line color 0=white 1=black, 2=red
			gr2->SetLineWidth(2);						
			gr2->SetMarkerColor(4);						
			gr2->SetMarkerStyle(20);
...
if (argc==6||argc>8)
{
	...
	if (col3==4)
		{
		TGraph *gr0d = new TGraph(i-1,xc,yc);		// Graph declaration
		gr0d->SetLineColor(6);					
