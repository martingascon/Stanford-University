...
#define N 20000 	// Maximum number of data to represent
...
Float_t xc[N],yc[N],dxc[N],dyc[N],y2c[N],dy2c[N],y3c[N],dy3c[N],y4c[N],dy4c[N],norm=1,norm2=1,norm3=1;
	      
Int_t i,k,m,col=1,col2=2,col3=2, col4=2, col5=2, col6=2;
...
// OUTPUT FILES
#if FILES
	ofstream *out1 = new ofstream("dtimes2.txt",ios::app);
	ofstream *out2 = new ofstream("amplis2.txt",ios::app);
#endif
...
if (col==2)
	...
	Float_t la=0,lb=0,HOff=-1.348e-07,HRes=2e-10,zero=-1*HOff/HRes;	
	//la=(HOff+(zero+1800)*HRes)*1000000;lb=(HOff+(zero+11900)*HRes)*1000000;
	la=0.6;lb=600;
	TF1 *f1 = new TF1("f1",fitf4,la,lb); 				
	f1->SetParameter(0,115);	f1->SetParameter(1,3.9);	
	f1->SetParameter(2,2.8); 	f1->SetParameter(3,1.24);	
	
//	f1->SetParLimits(1,20,25);
//	Float_t vf2,pos1,pos2,Int1,Int2;	
//	TF1 *f2 = new TF1("f2","gaus",320,600); 	// 300 for LaBr  320 for NaI
//	gr0->Fit("f2","NQR");
//	vf2=f2->GetParameter(1);
//	cout << vf2 << " vf2 " <<endl;
	
//	TF1 *f1 = new TF1("f1",fitf,370,750); 		// 300 for LaBr  320 for NaI	320-750 for CsI	
	//f1->SetParLimits(1,20,25);
	//gr0->Fit("f1","QR+");
	gr0->Fit("f1","R+");
	gr0->Fit("f1","R+");
	gr0->Fit("f1","R");
	d=f1->GetParameter(0);  	e=f1->GetParameter(1);  	f=f1->GetParError(0); 	g=f1->GetParError(1);	if (g<0.001) g=0.001;
	h=f1->GetParameter(3); 		l=f1->GetParError(3);		o=f1->GetParameter(2); 	p=f1->GetParError(2);				


	//pos1=f1->GetParameter(1);
	//pos2=f1->GetParameter(4);
	//Int1=f1->GetParameter(0);
	//Int2=f1->GetParameter(3);
	//ofstream *outx = new ofstream("intensities.txt",ios::app);
	//*outx << "0" << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << pos1 << "\t" << Int1 << "\t" << pos2 << "\t" << Int2 << endl;
	if (e<h) {
		aux = e; e = h; h = aux;
		aux = d; d = o; o = aux;
		aux = f; p = f; p = aux;
	...
	e=e/1000;g=g/1000;
	h=h/1000;l=l/1000;
	sepa= strtok(argv[1],"LaBr3Ce_355nm_50ms_1scan_PV_ kpsi");  sepa= strtok(argv[1],"LaBr3Ce_355nm_50ms_1scan_PV_ kpsi");
	//sepa= strtok(argv[1],"DTstab");  
	m=sprintf(latex1,"T = %s K",sepa);	
	m=sprintf(tau1,"#tau1 = (%.1f#pm %.1f) ns",1/e,g/(e*e)); 
	m=sprintf(tau2,"#tau2 = (%.1f#pm %.1f) ns",1/h,l/(h*h)); 

	m=sprintf(Amp1,"Amp1 = (%.1f#pm %.1f)",d/(d+o)*100,d/(d+o));	//sqrt(f*f/(d*d)+p*p/(o*o))*100); 
	m=sprintf(Amp2,"Amp2 = (%.1f#pm %.1f)",o/(d+o)*100,o/(d+o));		//sqrt(f*f/(d*d)+p*p/(o*o))*100); 

	//cout << "0" << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << max << "\t" << "0" << endl;
...
/*	tx->SetNDC(); 
	...
	tx->SetTextSize(0.04);
	tx->DrawLatex(0.7,0.90,tau1);
	tx->DrawLatex(0.7,0.85,tau2);
	tx->DrawLatex(0.7,0.75,Amp1);
	tx->DrawLatex(0.7,0.70,Amp2);

*/
#if FILES 
	//*out1 << "0" << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << max << "\t" << "0" << endl;
	*out1 << sepa << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << 1/e << "\t" << g/(e*e) <<"\t" << 1/h <<"\t" << l/(h*h) << endl;
	*out2 << sepa << "\t"  << "0.5" <<std::fixed << std::setprecision(1) <<"\t" << 1/h*o/(d+o)+ 1/e*d/(d+o)<< "\t" <<  d/(d+o)<<"\t" << o/(d+o)*100 <<"\t" << o/(d+o) << endl;
#endif
...
if (argc==6||argc>7)
{
	...
	if(!*in4) 
		...
		while (!in4->eof())
			{
			if (col4==2) *in4 >> xc[i] >> yc[i];
			if (col4==4) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i];  		//if (col==4) *in >> x[i] >> y[i] >> dx[i] >> dy[i];
			if (col4==5) *in4 >> xc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i];
			if (col4==6) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i];  
			if (col4==8) *in4 >> xc[i] >> dxc[i] >> yc[i] >> dyc[i] >> y2c[i] >> dy2c[i] >> y3c[i] >> dy3c[i];
	...
	if (col4==2)
		{
		TGraphErrors *gr0d = new TGraphErrors(i-1,xc,yc);		// Graph declaration
