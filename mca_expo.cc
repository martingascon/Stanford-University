...
Int_t i,k,m,a=1,b=8000,cont=0,reb=1;  
Float_t mean,sigma,dmean,dsigma,data[N],x[N],y[N],sig_left=1,sig_right=3; // [n] refers to a vector 		      
Float_t integral=0,r[10],dr[10];
char file[100],title[150]="",ci[100]=""; //title is a string with 150 characters
TH1I *H = new TH1I("H",file,N,1,N);  //TH =  histogram, 1 = 1 dimension, I = integer; H = name of histogram
...
ifstream *in = new ifstream(file); // open file in current directory
if(!*in) 
...
for (k=0;k<N;k++) // generate histogram's data    
	{
	x[k]=k;                 
	y[k]=data[k];
	H->Fill(x[k],y[k]);
...
Float_t par[5];
for (k=0;k<5;k++) par[k]=fitFcn->GetParameter(k);

TF1 *ex = new TF1("ex","[0]*TMath::Exp(-x*[1])",a,b);
ex->SetParameters(par[0],par[1]);	
ex->Draw("same");


TF1 *gaus = new TF1("gaus","[0]*TMath::Exp(-0.5*((x-[1])/[2])*((x-[1])/[2]))",a,b);
gaus->SetParameters(par[2],par[3],par[4]);	
gaus->Draw("same");
